...
public class JarModuleLoader implements ModuleLoader {
    ...
    public List<ModuleInfo> loadPackages(PreprocessConfig config) {
        ...
        for (String path : config.getJarPaths()) {
            final ModuleInfo info = loadPackage(new File(path), new JarModuleExportStrategy());
     ...
     */
    protected ModuleInfo loadPackage(final File file, final ModuleExportStrategy strategy) {
        ...
        try (JarFile jarFile = new JarFile(file)) {
            ...
            return types.isEmpty() ? null : new ModuleInfo(file, strategy, types);
        } catch (IOException e) {
